	...
	transformedStyles.stylesheet.rules.map(function(rule, ruleIndex) {
		if(rule.type === 'rule') {
			...
			rule.declarations = transformMap(rule.declarations, function(declaration, declarationIndex, declarations) {
				...
				if(transformedDeclaration.property === 'background-image' && /\?__sprite/i.test(transformedDeclaration.value)) {
					transformedDeclaration.value = transformedDeclaration.value.replace('?__sprite','');
					return cb(transformedDeclaration, declarationIndex, declarations);
				}
				// Background is a shorthand property so make sure `url()` is in there
				else if(transformedDeclaration.property === 'background' && /\?__sprite/i.test(transformedDeclaration.value)) {
					transformedDeclaration.value = transformedDeclaration.value.replace('?__sprite','');
